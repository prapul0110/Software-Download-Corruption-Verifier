...
(function () {
  ...
  function isolate() {
    ...
    if (introWordmark) {
      ...
      if (introText) {
        var fullText = introWordmark.text || 'SOFTWARE DOWNLOAD CORRUPTION VERIFIER';
        introText.innerHTML = '';
        var chars = [];
        for (var i = 0; i < fullText.length; i++) {
          var sp = document.createElement('span');
          sp.textContent = fullText[i] === ' ' ? '\u00a0' : fullText[i];
          sp.style.display = 'inline-block';
          introText.appendChild(sp);
          chars.push(sp);
        }
        var computedFs = Math.max(16, Math.min(42, Math.floor(window.innerWidth * 0.82 / (fullText.length * 0.58))));
        introText.style.fontSize = computedFs + 'px';
        introText.style.letterSpacing = '-0.015em';
...
export function TextAnimationCollection({
  ...
  const definition = useMemo<EffectDefinition>(() => {
    const baseDef = EFFECTS.threeUIIntro;
    const displayText = text || "SOFTWARE DOWNLOAD CORRUPTION VERIFIER";
    
    const len = displayText.length;
    let fontSize = 130;
    if (len > 30) fontSize = 42;
    else if (len > 20) fontSize = 58;
    else if (len > 10) fontSize = 80;

    return {
      ...baseDef,
      introWordmark: baseDef.introWordmark ? {
        ...baseDef.introWordmark,
        text: displayText,
        fontSize: fontSize,
      } : undefined
    };
